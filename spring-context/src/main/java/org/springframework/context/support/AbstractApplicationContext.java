...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			// Prepare this context for refreshing.准备刷新，设置一些系统属性
			 ...
			 * 这里的beanPostProcessor都是系统默认的，不是用户自定义的。比如负责注入
			 * ApplicationContext引用到各种Aware中的ApplicationContextAwareProcessor容器后处理器。
			 */
			...
			try {
				 ...
				 * 并可以修改它。同时可以定义多个BeanFactoryPostProcessor，通过设置'order'属性来确定各
				 * 个BeanFactoryPostProcessor执行顺序。这时候bean实例还没有创建，
				 * 但是不要把bean实例化
	 ...
	 * active flag as well as （同时）performing（执行） any initialization of property sources.
	 */
	...
	protected void initPropertySources() {
		// For subclasses: do nothing by default.被子类覆盖
	}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * 设置类加载器
		 * 我们知道 BeanFactory 需要加载类，也就需要类加载器，
		...
		/**
		 * 添加了一个（部分）处理aware相关接口的beanPostProcessor扩展，
		 * 主要是使用beanPostProcessor的postProcessBeforeInitialization()
		 * 前置处理方法实现aware相关接口的功能
		...
		/**
		 * 下面这些接口的实现类用户不能通过接口自动注入
		 * 设置了几个忽略自动装配的接口，默认只有BeanFactoryAware被忽略，其他的都要自行设置，
		 * 这里设置了ResourceLoaderAware、ApplicationEventPublisherAware、
		 * MessageSourceAware和ApplicationContextAware,EmbeddedValueResolverAware。
		 */
		...
		/**
		 * 这部分判断是否定义了名为loadTimeWeaver的bean，如果定义了则添加
		 * loadTimeWeaver功能的beanPostProcessor扩展，
		 * 并且创建一个临时的classLoader来让其处理真正的bean。
		 * spring的loadTimeWeaver主要是通过 instrumentation
		 * 的动态字节码增强在装载期注入依赖。
		 * AspectJ的支持
		 */
		...
		/**
		 * 这部分首先判断是否定义了名为systemProperties的bean，如果没有则加
		 * 载系统获取当前系统属性System.getProperties()并注册为一个单例bean。
		 * 假如有AccessControlException权限异常则创建一个ReadOnlySystemAttributesMap对象，
		 * 可以看到创建时重写了getSystemAttribute()方法，查看ReadOnlySystemAttributesMap
		 * 的代码可以得知在调用get方法的时候会去调用这个方法来获取key对应的对象，当获取依旧有权限
		 * 异常AccessControlException的时候则返回null。
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		 ...
		 * 编译期织入、类加载期织入和运行期织入。编译期织入是指在Java编译期，
		 * 采用特殊的编译器，将切面织入到Java类中；而类加载期织入则指通过特殊
		 * 的类加载器，在类字节码加载到JVM时，织入切面；运行期织入则是采用
		 * CGLib工具或JDK动态代理进行切面的织入。
		 * AspectJ采用编译期织入和类加载期织入的方式织入切面，
		 * 是语言级的AOP实现，提供了完备的AOP支持。它用AspectJ语
		 * 言定义切面，在编译期或类加载期将切面织入到Java类中。
		 * AspectJ提供了两种切面织入方式，第一种通过特殊编译器，
		 * 在编译期，将AspectJ语言编写的切面类织入到Java类中，
		 * 可以通过一个Ant或Maven任务来完成这个操作；第二种方式是
		 * 类加载期织入，也简称为LTW（Load Time Weaving）
		 */
	...
	protected void initMessageSource() {
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			//并且注入到容器中了，以后获取国际化配置文件的值的时候，可以自动注入MessageSource
	...
	protected void initApplicationEventMulticaster() {
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		/**
		 * 从BeanFactory中获取id为applicationEventMulticaster的ApplicationEventMulticaster（自己配置的）
		 */

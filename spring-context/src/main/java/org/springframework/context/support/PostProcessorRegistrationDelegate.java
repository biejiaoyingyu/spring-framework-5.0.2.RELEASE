...
class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		/**
		 * 首先判断一下传入的beanFactory是不是BeanDefinitionRegistry的实例，这个BeanDefinitionRegistry是可以注册Bean的
		 */
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			/**
			 * 然后处理我们手动注册的BeanFactoryPostProcessor，遍历每一个对象，
			 * 验证是不是BeanDefinitionRegistryPostProcessor的实例
			 */
			for (BeanFactoryPostProcessor postProcessor : beanFactoryPostProcessors) {
				/**
				 * 如果是BeanDefinitionRegistryPostProcessor的实例，那么就强制类型转换后，
				 * 执行BeanDefinitionRegistryPostProcessor的生命周期方法
				 * postProcessBeanDefinitionRegistry，并且将这个BeanFactoryPostProcessor
				 * 放入到registryPostProcessors容器中，如果不是BeanDefinitionRegistryPostProcessor
				 * 的实例，那么直接放入到regularPostProcessors容器中。
				 */
			 ...
			 * 如果是PriorityOrdered类型的，那么会放入priorityOrderedPostProcessors
			 * 容器中，然后排序，执行BeanDefinitionRegistryPostProcessor的生命周期方
			 ...
			 * 其中包括BeanDefinitionRegistryPostProcessor，是子接口
			 */
			...
			/**
			 * 如果是beanFactory instanceof BeanDefinitionRegistry，
			 * 如果不是的话，直接执行BeanFactoryPostProcessor的生命周期方
			 * 法postProcessBeanFactory，将手动添加的BeanFactoryPostProcessor处理掉
			 */
		...
		for (String ppName : postProcessorNames) {
			/**
			 * 跳过了第一次处理过了的后置处理器，需要debug才能确定一些真的东西
			 */
			if (processedBeans.contains(ppName)) {
		...
		/**
		 * 怎么感觉调用了两遍,这个是遍历了容器中所有的后置BeanFactoryPostProcessors
		 */
	...
	public static void registerBeanPostProcessors(
		...
		/**
		 * 后置处理器都默认可以通过PriorityOrdered、Ordered接口来执行优先级，也是按这个优先级来注册
		 */
		...
		for (String ppName : postProcessorNames) {
			if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
				//从容器中获取，获取不到就创建，然后加入到beanFatory中
